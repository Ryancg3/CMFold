...
mglobMin=function(data,parallel=FALSE,cl=NULL)
{
  ...
  if(parallel==FALSE)
  {
    return(matrix(apply(cbind(1:n),1,function(i) sqrt(colSums((t(sapply(data,function(x) Rfast::colMins(as.matrix(x),value=TRUE)))-t(sapply(data,function(x) Rfast::colMins(as.matrix(x),value=TRUE)))[,i])^2))),ncol=n,nrow=n,byrow=TRUE))
  } else {
    cl=parallel::makeCluster(cl)
    parallel::clusterExport(cl,list("data"))
    return(matrix(parallel::parApply(cl,cbind(1:n),1,function(i) sqrt(colSums((t(sapply(data,function(x) Rfast::colMins(as.matrix(x),value=TRUE)))-t(sapply(data,function(x) Rfast::colMins(as.matrix(x),value=TRUE)))[,i])^2))),ncol=n,nrow=n,byrow=TRUE))
